...
async function main() {
  ...
  const pdfOutputPath = path.join(__dirname, 'QR', 'QR-codes-all.pdf');
  ...
  console.log(`Génération de la planche A4 PDF avec ${urls.length} QR codes...`);
  ...
  doc.font('Helvetica-Bold').fontSize(18).text('Paris en chanteurs — Planche QR Codes', 0, 45, { align: 'center' });
  doc.font('Helvetica-Oblique').fontSize(9).fillColor('#555555').text('Collection Gallimard Voyage • Format unitaire : 2,5 x 2,5 cm', 0, 68, { align: 'center' });

  // Configuration de la grille
  const qrCm = 2.5;
  const ptPerCm = 72 / 2.54;
  const qrSize = qrCm * ptPerCm; // environ 70.866 points
  
  const columns = 4;
  const marginLeft = 65;
  const gapX = (595.28 - (marginLeft * 2) - (columns * qrSize)) / (columns - 1); // environ 60.6 points
